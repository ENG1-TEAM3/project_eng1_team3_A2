...
/**
 * The {@link MapHelper} class helps by setting up the map of the game, and
 * providing the {@link OrthogonalTiledMapRenderer} which is used to draw the
 * {@link TiledMap}.
 */
public class MapHelper {
	...
	/**
	 * The {@link MapHelper} constructor. It is {@code private} as it is a
	 * Singleton.
	 */
	private MapHelper() {
	}
	...
	/**
	 * Forgets the old {@link #INSTANCE}, and creates a new {@link #INSTANCE} of the
	 * {@link MapHelper}.
	 * 
	...
	/**
	 * Function to set the MapHelper's gameScreen so that it can access relevant
	 * information it may need to know.
	 * 
	...
	/**
	 * Sets up the map by loading the StationsMap tilemap, and then using the
	 * function {@link #parseMapObjects(MapObjects)} to parse and load it into the
	 * game.
	 * 
	...
	/**
	 * Creates a Static {@link Body} added to the map that is used to stop the
	 * {@link Cook} from moving through certain places.
	 * 
	...
	/**
	 * Creates a Polygon{@link Shape} using the {@link PolygonMapObject}. <br>
	 * It is used to create the {@link Shape} for the
	 * {@link #createStaticBody(PolygonMapObject)} function for the {@link Body}'s
	 * {@link com.badlogic.gdx.physics.box2d.Fixture}.
	 * 
	 * @param polygonMapObject
	 * @return
	 */
	 ...
	 * @param isStatic  If true, then the {@link Body} is stationary. If false, then
	 *                  the {@link Body} is not stationary.
	...
	public static Body makeBody(Rectangle rectangle, boolean isStatic) {
		return BodyHelper.createBody(rectangle.x + rectangle.getWidth() / 2, rectangle.y + rectangle.getHeight() / 2,
				rectangle.getWidth(), rectangle.getHeight(), isStatic, INSTANCE.gameScreen.getWorld());
	}

	/**
	 * Loops through all of the {@link MapObjects} and loads them into the
	 * {@link GameScreen}'s {@code world} using the other {@link MapHelper}
	 * functions.
	 * 
		...
		for (MapObject mapObject : mapObjects) {
			...
			if (mapObject instanceof RectangleMapObject) {
				...
				if (rectangleName.startsWith("Station")) {
					...
					case "cut":
						station = new PreparationStation(rectangle, locked, gameScreen);
					...
					case "fry":
						...
						break;
					case "counter":
						station = new CounterStation(rectangle, locked, gameScreen);
						station.setID(Station.StationID.counter);
						gameScreen.addGameEntity(station);
						break;
					case "bin":
						station = new BinStation(rectangle, locked, gameScreen);
					...
					case "serving":
						...
						break;
					default:
						station = new Station(rectangle, locked, gameScreen);
				...
				if (rectangleName.startsWith("Pantry")) {
					...
					Pantry pantry = new Pantry(rectangle, (boolean) mapObject.getProperties().get("locked"),
							gameScreen);
	...
	/**
	 * A dispose function to dispose of information when it is no longer needed.
	 */
	public void dispose() {
		tiledMap.dispose();
	}
